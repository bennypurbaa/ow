...
const FollowLike = async function (session, accountId) {
	var result;
	...
	if (result.length > 0) {
		...
		return chalk `{bold.green ${printFollow}:${printLike} » {bold.cyan Succsess [RUN]}}`;
	}
...
const Excute = async function (User, TargetUsername, Sleep, ittyw) {
	try {
		...
		console.log(chalk `{cyan ? Try to Follow and Like Followers Target . . . \n}`)
		const Targetfeed = new Client.Feed.AccountFollowers(doLogin.session, getTarget.id);
		var TargetCursor;
		console.log(chalk `{yellow ≡ READY TO START FNLAUTO WITH RATIO ${ittyw} TARGET/${Sleep} MiliSeconds\n}`)
		do {
			...
			for (let i = 0; i < TargetResult.length; i++) {
				...
				await Promise.all(TargetResult[i].map(async(akun) => {
					if (!getFollowers.includes(akun.id) && akun.params.isPrivate === false) {
						const ngeDo = await FollowLike(doLogin.session, akun.id)
...
console.log(chalk `{bold.cyan
  Ξ TITLE  : FNLAUTO [F-L with Followers Target]
