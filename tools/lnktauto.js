...
const CommentAndLike = async function (session, accountId, text) {
	...
	if (result.length > 0) {
		...
		return chalk `{bold.green ${printLike}:${printComment} » {bold.cyan ${text}}}`;
	}
...
const Excute = async function (User, TargetUsername, Text, Sleep, ittyw) {
	try {
		...
		console.log(chalk `{cyan ? Try to Like & Comment Followers Target . . . \n}`)
...
console.log(chalk `{bold.cyan
  Ξ TITLE  : LNKTAUTO [L-C with Followers Target]
